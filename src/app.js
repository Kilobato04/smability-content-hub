...
function buildSlideEl(slide, index, total, sidepx, bgBase64) {
    ...
    // --- FIX DE DIMENSIONAMIENTO ---
    // Forzamos que en el Hub siempre sea cuadrado y no se colapse como tira
    if (!isPDF) {
        el.style.width = '100%';
        el.style.aspectRatio = '1 / 1';
        el.style.minHeight = el.offsetWidth + 'px'; // Asegura que no sea una tira
    }
    ...
    const fs_h3 = isPDF ? Math.round(S * 0.07) : 38; 
    ...
    if (slide.type === 'cover_bold') {
        ...
        bodyContent = `
            ...
            <div style="position:absolute; bottom:${pad * 1.5}px; left:${pad}px; right:${pad}px; z-index:10;">
                ...
                ${slide.ai_hook ? `<p style="border-left:3px solid #39FF14; padding-left:20px; margin-top:20px; color:#9A9A9A; font-weight:600; font-size:18px; line-height:1.4;">${slide.ai_hook}</p>` : ''}
            </div>`;

    } else if (slide.type === 'data_callout') {
        const deviceCircleSize = isPDF ? Math.round(S * 0.12) : 85;
        bodyContent = `
            <div style="position:absolute; top:${pad}px; left:${pad}px; right:${pad}px; z-index:10;">
                <p style="font-family:'Space Grotesk'; font-size:13px; color:#0047AB; font-weight:800; letter-spacing:2px; margin-bottom:10px; text-shadow: 0 1px 2px rgba(255,255,255,0.5);">ANÁLISIS TÉCNICO // 2026</p>
                <h3 style="font-size:28px; color:#111; font-weight:900; line-height:1.1; text-transform:uppercase; max-width:65%;">${slide.headline}</h3>
            </div>
            <div class="device-circle" style="position:absolute; top:${pad}px; right:${pad}px; width:${deviceCircleSize}px; height:${deviceCircleSize}px; background:#222; border:2px solid #39FF14; border-radius:50%; display:flex; align-items:center; justify-content:center; overflow:hidden; z-index:11;">
            ...
            <div class="tech-specs-box" style="position:absolute; bottom:${pad}px; right:0; width:200px; background:rgba(255,255,255,0.95); border-left:6px solid #39FF14; padding:15px; color:#111; z-index:10; box-shadow: -5px 5px 15px rgba(0,0,0,0.1);">
                ...
                <p style="font-family:'Inter'; font-size:12px; font-weight:700; line-height:1.3; margin:0; color:#333;">${(slide.technical_specs || '').replace(/\n/g, '<br>')}</p>
            </div>`;

    } else if (slide.type === 'bullets') {
        const bulletItems = (slide.bullets || []).map((b, i) => `
            <div style="display:flex; align-items:flex-start; gap:15px; padding:12px 0; border-bottom:1px solid rgba(255,255,255,0.1);">
                <span style="color:#39FF14; font-family:'Space Grotesk'; font-weight:900; font-size:16px;">0${i+1}</span>
                <span style="color:#fff; font-weight:600; font-size:18px; line-height:1.3;">${b}</span>
            </div>`).join('');
        bodyContent = `
            <div style="position:absolute; top:${pad}px; left:${pad}px; right:${pad}px; z-index:10;">
                <h3 style="font-size:26px; font-weight:900; color:#fff; margin-bottom:20px;">${slide.headline}</h3>
        ...
        bodyContent = `
            <div style="position:absolute; top:50%; left:${pad}px; right:${pad}px; transform:translateY(-50%); z-index:10;">
                <h3 style="font-size:34px; font-weight:900; color:#fff; margin-bottom:20px; text-transform:uppercase; line-height:1.1;">${slide.headline}</h3>
                <p style="font-size:18px; color:#F0F0F0; border-left:5px solid #39FF14; padding-left:25px; line-height:1.4; font-weight:600;">${slide.ai_body}</p>
                ${slide.metric ? `<div style="font-size:64px; font-weight:900; color:#39FF14; margin-top:20px; letter-spacing:-2px;">${slide.metric}</div>` : ''}
            </div>`;
        ...
        bodyContent = `
            <div style="position:absolute; inset:${pad}px; display:flex; flex-direction:column; justify-content:center; z-index:10;">
                <h3 style="font-size:44px; font-weight:900; margin-bottom:30px; line-height:1;">${slide.headline}</h3>
                <div style="display:flex; align-items:center; gap:20px; background:rgba(255,255,255,0.05); padding:15px; border-radius:10px; border:1px solid rgba(255,255,255,0.1);">
                    <div style="width:100px; height:100px; background:#fff; padding:8px; border-radius:8px; flex-shrink:0;">
                        <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=https://smability.io/aire/gpt.html" style="width:100%;">
                    </div>
                    <div>
                        <p style="color:#fff; font-weight:800; font-size:18px; margin:0;">AIreGPT v2.6</p>
                        <p style="color:rgba(255,255,255,0.6); font-weight:600; font-size:13px; margin-top:5px; line-height:1.2;">Escanea para probar el monitoreo en tiempo real.</p>
                    </div>
                </div>
            </div>`;
    }

    const numLabel = `<span style="position:absolute; top:18px; left:24px; font-family:'Space Grotesk'; font-size:11px; font-weight:700; color:rgba(255,255,255,0.4); z-index:20; background:rgba(0,0,0,0.4); padding:3px 8px; border-radius:4px;">0${index + 1} / 0${total}</span>`;

    const footer = slide.type === 'data_callout' ? '' : `
        <div style="position:absolute; bottom:${pad/2}px; left:${pad}px; right:${pad}px; border-top:1px solid rgba(255,255,255,0.1); padding-top:10px; font-family:'Space Grotesk'; font-size:12px; color:rgba(255,255,255,0.4); font-weight:700; display:flex; align-items:center; gap:8px; z-index:20;">
